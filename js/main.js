	...
	$.get('main.php?global_variables', function(xhr_data)
	{
		...
		if(ua_supports_touch)
		{
			...
			$(document).on(pointer_move_event, function(event)
			{
				...
				if(pointer_start_x < pointer_edge || isDisplayed('div#transparent_cover_div') || isDisplayed('div#black_cover_div') || isDisplayed('div#black_cover_activity_div') || isVisible('div#nowplaying_div'))
				{
		...
		$(document).on(pointer_event, 'div.actions_div, span.actions_span', function()
		{
			...
			for(var i = 0; i < actions.length; i++)
			{
				...
				{
					if(!isVisible('div#menu_div')) scrollToTop(true);
				}
		...
		// Volume slider
		$(document).on('input', 'input#nowplaying_volume_slider', function()
		{
			var element = this;
			var value = $(element).val();
