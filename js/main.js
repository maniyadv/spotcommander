...
function setGlobalVariables(global_variables)
{
	...
	// Feature detection
	ua_supports_csstransitions = (Modernizr.csstransitions);
	ua_supports_csstransforms3d = (Modernizr.csstransforms3d);
	ua_supports_devicemotion = (Modernizr.devicemotion);
	ua_supports_inputtype_range = (Modernizr.inputtypes.range);
	ua_supports_touch = (Modernizr.touch);
	...
	var settings = [
		...
		{ setting: 'settings_start_track_radio_fix' , value: 'false' },
	...
	settings_start_track_radio_fix = stringToBoolean($.cookie('settings_start_track_radio_fix'));
...
$(window).load(function()
{
	...
	$.get('main.php?global_variables', function(xhr_data)
	{
		...
		if(ua_supports_touch)
		{
			$(document).on(pointer_down_event, function(e)
			{
				pointer_points = e.originalEvent.touches.length;
				pointer_is_down = true;
				pointer_moved = (pointer_points != 1);
				pointer_gesture_done = false;

				pointer_start_x = e.originalEvent.touches[0].pageX;
				pointer_start_y = e.originalEvent.touches[0].pageY;

				pointer_edge = 25;
			});

			$(document).on(pointer_move_event, function(e)
			{
				if(typeof pointer_start_x == 'undefined' || typeof pointer_start_y == 'undefined') return;

				pointer_end_x = e.originalEvent.touches[0].pageX;
				pointer_end_y = e.originalEvent.touches[0].pageY;
				...
				{
					if($(e.target).attr('id') != 'nowplaying_volume_slider') e.preventDefault();
				}
				...
				if(ua_is_ios && ua_is_standalone)
				{
					...
					if(window_height > activity_height && !isVisible('div#nowplaying_div')) e.preventDefault();
		...
		else
		{
			...
			$(document).on(pointer_move_event, function(e)
			{
				...
				pointer_end_x = e.pageX;
				pointer_end_y = e.pageY;
		...
		if(ua_supports_touch)
		{
			$(document).on(pointer_move_event, 'div#top_actionbar_div, div#bottom_actionbar_div', function(e)
			{
				e.preventDefault();
			});

			$(document).on(pointer_down_event, 'div#bottom_actionbar_div', function(e)
			{
				pointer_bottombar_start_x = e.originalEvent.touches[0].pageX;
				pointer_bottombar_start_y = e.originalEvent.touches[0].pageY;
			});

			$(document).on(pointer_move_event, 'div#bottom_actionbar_div', function(e)
			{
				pointer_bottombar_end_x = e.originalEvent.touches[0].pageX;
				pointer_bottombar_end_y = e.originalEvent.touches[0].pageY;
			...
			$(document).on(pointer_down_event, 'div#nowplaying_div', function(e)
			{
				pointer_nowplaying_start_x = e.originalEvent.touches[0].pageX;
				pointer_nowplaying_start_y = e.originalEvent.touches[0].pageY;
			});

			$(document).on(pointer_move_event, 'div#nowplaying_div', function(e)
			{
				pointer_nowplaying_end_x = e.originalEvent.touches[0].pageX;
				pointer_nowplaying_end_y = e.originalEvent.touches[0].pageY;
				...
				if(!pointer_gesture_done && !nowplaying_cover_art_moving && $(e.target).attr('id') != 'nowplaying_volume_slider' && pointer_nowplaying_moved_y > gesture_trigger && Math.abs(pointer_nowplaying_moved_x) < gesture_block)
				{	
					hideNowplaying();
					pointer_gesture_done = true;
				}
			...
			$(document).on(pointer_down_event, 'div#nowplaying_cover_art_div', function(e)
			{
				pointer_cover_art_start_x = e.originalEvent.touches[0].pageX;
				pointer_cover_art_start_y = e.originalEvent.touches[0].pageY;
			...
			$(document).on(pointer_move_event, 'div#nowplaying_cover_art_div', function(e)
			{
				pointer_cover_art_end_x = e.originalEvent.touches[0].pageX;
				pointer_cover_art_end_y = e.originalEvent.touches[0].pageY;
			...
			$(document).on(pointer_up_event, 'div#nowplaying_cover_art_div', function(e)
			{
				nowplaying_cover_art_moving = false;
		...
		if(settings_motion_gestures && ua_supports_devicemotion)
		{
			...
			{
				motion_x1 = e.accelerationIncludingGravity.x, motion_y1 = e.accelerationIncludingGravity.y, motion_z1 = e.accelerationIncludingGravity.z;
			}, false);
		...
		$(document).on(pointer_event, 'div.actions_div, span.actions_span', function()
		{
			...
			for(var i = 0; i < actions.length; i++)
			{
				...
				else if(action == 'show_dialog_actions')
				{
					showDialogActions(data.dialogactions);					
				}
				...
				else if(action == 'change_native_app_url')
				{
					changeNativeAppUrl()
				}
				...
				{
					queueUris(data.uri, data.randomly);
				}
		...
		// Text fields
		$(document).on('focus blur', 'input:text', function()
		{
			var element = this;
