...
function activityLoaded()
{
	...
	// Cards
	if($('div.card_div').length)
	{
		if(ua_supports_csstransitions && ua_supports_csstransforms3d) $('div.card_div').addClass('card_shadow_div');

		if($('div.card_cover_div').length) $('div.card_cover_div').width($('div.card_cover_div').outerHeight());
	}
...
function showMenu()
{
	...
	$('div.menu_big_item_indicator_div').css('visibility', '');
	...
	$('div.menu_big_item_div').each(function()
	{
		...
		{
			$('div.menu_big_item_text_div', item).css('font-weight', 'bold');
			$('div.menu_big_item_indicator_div', item).css('visibility', 'visible');
		}
	});
	...
	setTimeout(function()
	{
		...
		if(ua_supports_csstransitions) $('div#black_cover_activity_div').addClass('show_black_cover_activity_div_animation');
	}, 25);
...
function hideMenu()
{
	...
	{
		var left = $('div#menu_div').data('cssleft');

		$('div#menu_div').stop().animate({ left: left }, 250, 'easeOutExpo', function()
		{
	...
	{
		$('div#black_cover_activity_div').hide();
	}
}
...
{
	var cookie = { id: 'hide_shuffle_play_uri_dialog_'+project_version, value: 'true', expires: 36500 };
	...
	xhr_nowplaying = $.get('nowplaying.php', function(xhr_data)
	{
		...
		if(type == 'manual' || nowplaying_last_data != xhr_data)
		{
			...
			if(type == 'manual' || nowplaying_last_uri != nowplaying.uri)
			{
				...
				{
					$.getJSON('http://www.olejon.net/code/spotcommander/api/1/cover-art/?type=album&artist='+encodeURIComponent(nowplaying.artist)+'&album='+encodeURIComponent(nowplaying.album)+'&callback=?', function(data)
					{
						var lastfm_cover_art = (typeof data.mega == 'undefined') ? 'img/no-cover-art-640.png?'+project_serial : data.mega;
...
function setCoverArtSize()
{
	...
	if(cover_art_width > window_width)
	{
		var ratio = window_width / cover_art_width;
		var cover_art_height = Math.floor(cover_art_height * ratio);
		var minimum_height = $('div#cover_art_art_div').height();

		var size = (cover_art_height < minimum_height) ? 'auto '+minimum_height+'px' : window_width+'px auto';
...
{
	showDialog({ title: 'Import from Spotify', body_class: 'dialog_message_div', body_text: 'This is very experimental. You may have to try several times for it to work, and when it does, it may take some time.<br><br>Continue?', button1: { text: 'No', keys : ['actions'], values: ['hide_dialog'] }, button2: { text: 'Yes', keys : ['actions'], values: ['hide_dialog import_starred_spotify_tracks'] } });
}
...
function browseArtist(uri)
{
	...
	{
		changeActivity('artist', '', 'uri='+uri+'&region='+settings_region);
	}
}
...
function checkForDialogs()
{
	...
	{
		var cookie = { id: 'hide_ox_x_integration_dialog_'+project_version, value: 'true', expires: 36500 };
