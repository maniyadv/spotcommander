...
function showActivity()
{
	...
	{
		var cookie = { id: 'current_activity_'+project_serial, value: JSON.stringify({ activity: activity.activity, subactivity: activity.subactivity, args: activity.args }), expires: 1 };
...
function activityLoaded()
{
	...
	else
	{
		...
		{
			setActivityActions('<div title="'+data.actions[0].action[0]+'" class="actions_div" data-highlightclass="green_highlight" onclick="void(0)"><div class="img_div img_32_div '+data.actions[0].action[1]+'"></div></div>');
		...
		{
			setActivityActions('<div title="More" class="actions_div" data-actions="show_activity_overflow_actions" data-highlightclass="green_highlight" onclick="void(0)"><div class="img_div img_32_div overflow_32_img_div"></div></div>');
			...
			for(var i = 0; i < data.actions.length; i++)
			{
				...
				$('div#top_actionbar_overflow_actions_inner_div').append('<div class="actions_div" data-highlightclass="green_highlight" '+highlight_arrow+' onclick="void(0)">'+data.actions[i].action[0]+'</div>');
	...
	if(typeof data.cover != 'undefined' && data.cover != '') getCoverArt(data.cover);
...
{
	var cookie = { id: 'hide_first_time_activity_'+project_serial, value: 'true', expires: 36500 };
...
{
	var cookie = { id: 'hide_adjust_volume_control_dialog_'+project_serial, value: 'true', expires: 36500 };
...
{
	var cookie = { id: 'hide_shuffle_repeat_dialog_'+project_serial, value: 'true', expires: 36500 };
...
{
	var cookie = { id: 'hide_play_uri_randomly_dialog_'+project_serial, value: 'true', expires: 36500 };
...
{
	var cookie = { id: 'hide_start_track_radio_dialog_'+project_serial, value: 'true', expires: 36500 };
...
function refreshNowplaying(type)
{
	...
	xhr_nowplaying = $.get('nowplaying.php', function(xhr_data)
	{
		...
		if(type == 'manual' || nowplaying_last_data != xhr_data)
		{
			...
			if(nowplaying.actions.length == 1)
			{
				$('div#nowplaying_actionbar_right_div').html('<div title="'+nowplaying.actions[0].action[0]+'" class="actions_div" data-highlightclass="green_highlight" onclick="void(0)"><div class="img_div img_32_div '+nowplaying.actions[0].action[1]+'"></div></div>');

				for(var i = 0; i < nowplaying.actions[0].keys.length; i++)
				{
					$('div.actions_div', 'div#nowplaying_actionbar_right_div').data(nowplaying.actions[0].keys[i], nowplaying.actions[0].values[i]);
				}
			}
			else
			{
				$('div#nowplaying_actionbar_right_div').html('<div title="More" class="actions_div" data-actions="show_nowplaying_overflow_actions" data-highlightclass="green_highlight" onclick="void(0)"><div class="img_div img_32_div overflow_32_img_div"></div></div>');
				...
				for(var i = 0; i < nowplaying.actions.length; i++)
				{
					...
					$('div#nowplaying_actionbar_overflow_actions_inner_div').append('<div class="actions_div" data-highlightclass="green_highlight" '+highlight_arrow+' onclick="void(0)">'+nowplaying.actions[i].action[0]+'</div>');
			...
			{
				$.getJSON('http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=f1e3c1566b4b0804dcd36683f298292f&artist='+encodeURIComponent(nowplaying.artist)+'&album='+encodeURIComponent(nowplaying.album)+'&format=json&callback=?', function(data)
				{
					var lastfm_cover_art = (typeof data.album == 'undefined') ? 'img/no-cover-art-640.png?'+project_serial : data.album.image[4]['#text'];
...
function importStarredTracks(uris)
{
	...
	for(var i = 0; i < validate.length; i++)
	{
		var type = getUriType(validate[i]);
		if(type != 'track' && type != 'local') invalid = true;
	}
...
{
	if(ua_is_ios && ua_is_standalone) $.removeCookie('current_activity_'+project_serial);
...
function checkForDialogs()
{
	...
	if(!ua_is_supported)
	{
		var cookie = { id: 'hide_unsupported_browser_dialog_'+project_serial, value: 'true', expires: 7 };
	...
	if(!ua_supports_csstransitions || !ua_supports_csstransforms3d)
	{
		var cookie = { id: 'hide_software_accelerated_animations_dialog_'+project_serial, value: 'true', expires: 36500 };
	...
	{
		var cookie = { id: 'hide_update_available_dialog_'+project_serial, value: 'true', expires: 7 };
	...
	if(ua_is_android)
	{
		if(ua_is_android_app)
		{
			var cookie = { id: 'hide_android_app_versions_mismatch_dialog_'+project_serial, value: 'true', expires: 1 };
			...
			var cookie = { id: 'hide_android_hardware_buttons_dialog_'+project_serial, value: 'true', expires: 36500 };
			...
			var cookie = { id: 'hide_rate_on_google_play_dialog_'+project_serial, value: 'true', expires: 36500 };
		...
		{
			var cookie = { id: 'hide_android_app_dialog_'+project_serial, value: 'true', expires: 1 };
	...
	else if(ua_is_ios)
	{
		if(ua_is_standalone)
		{
			var cookie = { id: 'hide_ios_back_gesture_dialog_'+project_serial, value: 'true', expires: 36500 };
		...
		{
			var cookie = { id: 'hide_ios_home_screen_dialog_'+project_serial, value: 'true' };
	...
	{
		var cookie = { id: 'hide_ubuntu_unity_integration_dialog_'+project_serial, value: 'true', expires: 36500 };
	...
	{
		var cookie = { id: 'hide_windows_desktop_integration_dialog_'+project_serial, value: 'true', expires: 36500 };
...
function nativeAppLoad(is_paused)
{
	if(ua_is_android_app)
	{
		...
		if(shareUri != '')
		{
			...
			if(type == 'playlist')
			{
				changeActivity('playlists', 'browse', 'uri='+shareUri);
			}
