...
if(isset($_GET['search']))
{
	...
	else
	{
		...
		$total_results = 50;
		$initial_results = (is_array($tracks) && is_array($albums)) ? 5 : 50;
		...
		if(is_array($tracks))
		{
			...
			foreach($tracks as $track)
			{
				...
				if(is_available_in_region($region, $album_regions))
				{
					$i++;
		...
		if(is_array($albums))
		{
			$actions_dialog = array();
			$actions_dialog['title'] = 'Sort by';
			$actions_dialog['actions'][] = array('text' => 'Popularity', 'keys' => array('actions', 'cookieid', 'cookievalue', 'cookieexpires'), 'values' => array('hide_dialog set_cookie refresh_activity', 'settings_sort_search_albums', 'default', 36500));
