...
define('daemon_socket', '/tmp/spotcommander-' . daemon_user() . '.socket');
...
if(isset($_POST['action']))
{
	...
	elseif($action == 'play_uri' || $action == 'play_uri_randomly' || $action == 'start_track_radio')
	{
		clear_queue();
		remote_control($action, $data);
	}
	elseif($action == 'play_artist')
	{
		clear_queue();
		echo play_artist($data);
	}
...
{
	echo json_encode(array(project_name, project_version, project_serial, project_website, project_developer, project_android_app_minimum_version, check_for_errors()));
}
