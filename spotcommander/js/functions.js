...
function activityLoaded()
{
	...
	if(typeof data.cover_art != 'undefined') getCoverArt(data.cover_art);
...
{
	var args = args.replace(/\&amp\;/g, '&').replace(/%2F/g, '%252F').replace(/%5C/g, '%255C');
...
function remoteControl(action)
{
	...
	if(action == 'launch_quit' || action == 'next' || action == 'previous') prepareRefreshNowplaying();
...
function startTrackRadio(uri, play_first)
{
	...
	{
		showDialog({ title: 'Start track radio', body_class: 'dialog_message_div', body_text: 'For this to work, you must leave the mouse cursor over the now playing cover art (avoid the resize button) on the computer running Spotify, or it will fail badly. If it does not work, check out the fix in settings. It may not work on all tracks.<br><br>Continue?', button1: { text: 'No', keys : ['actions'], values: ['hide_dialog'] }, button2: { text: 'Yes', keys: ['actions', 'cookieid', 'cookievalue', 'cookieexpires', 'uri', 'playfirst'], values: ['hide_dialog set_cookie start_track_radio', cookie.id, cookie.value, cookie.expires, uri, play_first] } });
	}
	else
	{
...
function prepareRefreshNowplaying()
{
	...
	$('div#nowplaying_actionbar_right_div > div').css('pointer-events', 'none').css('opacity', '0.5');
...
function refreshNowplaying(type)
{
	...
	xhr_nowplaying = $.get('nowplaying.php', function(xhr_data)
	{
		...
		if(type != 'silent' || nowplaying_last_data != xhr_data)
		{
			...
			$('div#nowplaying_actionbar_right_div > div').css('pointer-events', '').css('opacity', '');
			...
			$('div#nowplaying_cover_art_div').attr('title', nowplaying.album+' ('+nowplaying.year+')').css('background-image', 'url("'+nowplaying.cover_art+'")');
			...
			$('div#nowplaying_cover_art_div').data('uri', nowplaying.uri);
			...
			if(type != 'silent' || nowplaying_last_uri != nowplaying.uri)
			{
...
{
	if(!isActivity('recently-played', '', '')) return;

	$.get('recently-played.php', function(xhr_data)
	{
		if(isActivity('recently-played', '', '')) setActivityContent(xhr_data);
	});
}
...
function getQueue()
{
	if(!isActivity('queue', '', '')) return;

	$.get('queue.php', function(xhr_data)
	{
		if(isActivity('queue', '', '')) setActivityContent(xhr_data);
	});
}
...
function queueUris(uri, randomly)
{
	showToast('Queuing tracks', 2);

	$.post('queue.php?queue_uris&'+getCurrentTime(), { uri: uri, randomly: randomly }, function(xhr_data)
	{
		setTimeout(function()
		{
			if(xhr_data == 'spotify_is_not_running')
			...
			{
				showToast(xhr_data+' tracks queued', 2);
				getQueue();
			}
		}, 1000);
	});
...
{
	$.post('queue.php?remove&'+getCurrentTime(), { id: id }, function()
	{
		getQueue();
	});	
}
...
{
	$.get('playlists.php', function(xhr_data)
	{
		setActivityContent(xhr_data);
	});
}
...
function getStarred()
{
	if(!isActivity('starred', '', '')) return;

	$.get('starred.php', function(xhr_data)
	{
		if(isActivity('starred', '', '')) setActivityContent(xhr_data);
	});
}
...
{
	if(!isActivity('search', 'history', '')) return;

	$.get('search.php?history', function(xhr_data)
	{
		if(isActivity('search', 'history', '')) setActivityContent(xhr_data);
	});
}
...
function showDialog(dialog)
{
	...
	setTimeout(function()
	{
		$('div#dialog_div').html('<div id="dialog_header_div">'+dialog.title+'</div><div id="dialog_body_div"><div id="'+dialog.body_class+'">'+dialog.body_text+'</div></div><div id="dialog_buttons_div"><div id="dialog_button1_div" class="actions_div" data-highlightclass="green_highlight" onclick="void(0)">'+dialog.button1.text+'</div></div>');
...
function showDialogActions(actions)
{
	var actions = $.parseJSON($.base64.decode(actions));
	var body = '';
	...
	for(var i = 0; i < actions.length; i++)
	{
		...
		body += '<div class="actions_div" '+data+' data-highlightclass="green_highlight" onclick="void(0)">'+actions[i].text+'</div>';
	}

	showDialog({ title: 'More', body_class: 'dialog_actions_div', body_text: body, button1: { text: 'Close', keys : ['actions'], values: ['hide_dialog'] } });
}
