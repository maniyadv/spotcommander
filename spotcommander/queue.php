...
{
	echo queue_uris($_POST['uri'], $_POST['randomly']);
}
elseif(isset($_GET['remove']))
{
	echo remove_from_queue($_POST['id']);
}
...
else
{
	...
	$activity['actions'] = array('icon' => array('Clear', 'remove_32_img_div'), 'keys' => array('actions'), 'values' => array('clear_queue'));

	$tracks = get_db_rows('queue', "SELECT * FROM queue ORDER BY id", array('id', 'artist', 'title', 'uri'));
	...
	else
	{
		...
		foreach($tracks as $track)
		{
			...
			echo '
				...
				<div title="Remove" class="actions_div" data-actions="remove_from_queue" data-id="' . $id . '" data-highlightclass="dark_grey_highlight" data-highlightotherelement="div.list_item_main_actions_arrow_div" data-highlightotherelementparent="div.list_item_div" data-highlightotherelementclass="up_arrow_dark_grey_highlight" onclick="void(0)"><div class="img_div img_24_div remove_24_img_div"></div></div>
				</div>
				</div>
				</div>
			';
