...
{
	file_write(__DIR__ . '/run/volume-before-mute.txt', $volume);
}
...
function get_cover_art($uri)
{
	...
	if(!$cover_art)
	{
		...
		{
			$cover = $cover[1];
			$cover = preg_replace(array('/\/thumb\//', '/\/image\//', '/\/300\//', '/^https:\/\/\w+\.cloudfront\.net/'), array('/640/', '/640/', '/640/', 'http://o.scdn.co'), $cover);
		...
		if($search == 'tracks' && isset($tracks['tracks']) || $search == 'albums' && isset($albums['albums']) || $search == 'all' && isset($tracks['tracks']) && isset($albums['albums']))
		{
			...
			if(!empty($tracks))
			{
				...
				foreach($tracks as $track)
				{
					...
					$return['tracks'][$i]['artist_uri'] = (empty($track['artists'][0]['href'])) ? '' : $track['artists'][0]['href'];
					$return['tracks'][$i]['title'] = $track['name'];
					$return['tracks'][$i]['length'] = convert_length($track['length'], 's');
					$return['tracks'][$i]['popularity'] = convert_popularity($track['popularity']);
					$return['tracks'][$i]['uri'] = $track['href'];
					$return['tracks'][$i]['album_uri'] = $track['album']['href'];
					$return['tracks'][$i]['album_regions'] = $track['album']['availability']['territories'];
			...
			if(!empty($albums))
			{
				...
				foreach($albums as $album)
				{
					...
					$return['albums'][$i]['artist_uri'] = (empty($album['artists'][0]['href'])) ? '' : $album['artists'][0]['href'];
					$return['albums'][$i]['title'] = $album['name'];
					$return['albums'][$i]['popularity'] = convert_popularity($album['popularity']);
					$return['albums'][$i]['uri'] = $album['href'];
					$return['albums'][$i]['regions'] = $album['availability']['territories'];
...
function lookup_album($uri)
{
	...
	else
	{
		...
		if(!empty($album['album']['name']) && !empty($album['album']['tracks']))
		{
			...
			$return['artist_uri'] = (empty($album['album']['artist-id'])) ? '' : $album['album']['artist-id'];
			$return['title'] = $album['album']['name'];
			$return['released'] = (empty($album['album']['released'])) ? 'Unknown' : $album['album']['released'];
			...
			foreach($tracks as $track)
			{
				...
				$return['discs'][$disc_number][$i]['artist_uri'] = $track['artists'][0]['href'];
				$return['discs'][$disc_number][$i]['title'] = $track['name'];
				$return['discs'][$disc_number][$i]['track_number'] = $track['track-number'];
				$return['discs'][$disc_number][$i]['disc_number'] = $track['disc-number'];
				$return['discs'][$disc_number][$i]['length'] = convert_length($track['length'], 's');
				$return['discs'][$disc_number][$i]['popularity'] = convert_popularity($track['popularity']);
				$return['discs'][$disc_number][$i]['uri'] = $track['href'];

				$total_length = $total_length + floatval($track['length']);

				$i++;
			}

			$return['tracks_count'] = count($tracks);
			$return['total_length'] = convert_length($total_length, 's');
...
function lookup_artist($uri)
{
	...
	if(empty($return))
	{
		$files = get_external_files(array('https://embed.spotify.com/?uri=' . $uri, 'http://ws.spotify.com/lookup/1/.json?uri=' . $uri . '&extras=albumdetail'));
		$artist_tracks = $files[0];
		$artist_albums = json_decode($files[1], true);

		preg_match_all('/<li class="artist .*>(.*?)<\/li>/', $artist_tracks, $tracks_artists);
		preg_match_all('/<li class="track-title .*>(.*?)<\/li>/', $artist_tracks, $tracks_titles);
		preg_match_all('/data-duration-ms="(.*?)"/', $artist_tracks, $tracks_lengths);
		preg_match_all('/rel="track" data-track="(.*?)"/', $artist_tracks, $tracks_uris);

		if(!empty($tracks_artists[1]) && !empty($tracks_titles[1]) && !empty($tracks_lengths[1]) && !empty($tracks_uris[1]) && !empty($artist_albums['artist']['name']) && !empty($artist_albums['artist']['href']) && !empty($artist_albums['artist']['albums']))
		{
			$return = array();
			$return['artist'] = $artist_albums['artist']['name'];
			$return['uri'] = $artist_albums['artist']['href'];

			$count = count($tracks_uris[1]);

			for($i = 0; $i < $count; $i++)
			{
				$return['tracks'][$i]['artist'] = hscd($tracks_artists[1][$i]);
				$return['tracks'][$i]['title'] = hscd(preg_replace('/^\d+\. /', '', $tracks_titles[1][$i]));
				$return['tracks'][$i]['length'] = convert_length($tracks_lengths[1][$i], 'ms');
				$return['tracks'][$i]['uri'] = 'spotify:track:' . $tracks_uris[1][$i];
			}

			$albums = $artist_albums['artist']['albums'];

			$i = 0;

			foreach($albums as $album)
			{
				$type = ($album['album']['artist'] == $artist_albums['artist']['name']) ? 'albums' : 'appears_on_albums';

				$return[$type][$i]['artist'] = $album['album']['artist'];
				$return[$type][$i]['artist_uri'] = (empty($album['album']['artist-id'])) ? '' : $album['album']['artist-id'];
				$return[$type][$i]['title'] = $album['album']['name'];
				$return[$type][$i]['released'] = (empty($album['album']['released'])) ? 'Unknown' : $album['album']['released'];
				$return[$type][$i]['uri'] = $album['album']['href'];

				$i++;
			}

			$return['albums_count'] = (empty($return['albums'])) ? 0 : count($return['albums']);
...
function lookup_track_artist($uri)
{
	...
	{
		$tracks = lookup_tracks(array($uri));
		$return = (empty($tracks[0]['track']['artists'][0]['href'])) ? null : $tracks[0]['track']['artists'][0]['href'];
	}
...
function check_for_updates()
{
	...
	$files = get_external_files(array(project_website . 'latest-version.php?version=' . rawurlencode(number_format(project_version, 1)) . '&uname=' . rawurlencode($sysinfo['uname']) . '&ua=' . rawurlencode($sysinfo['ua'])));
...
function get_albums_count($count)
{
	return ($count > 1) ? $count . ' albums' : $count . ' album';
}
