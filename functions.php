...
function get_spotify_user_path()
{
...
function play_artist($uri)
{
	...
	elseif(get_uri_type($uri) == 'album')
	{
		...
		if(!empty($album['artist_uri'])) $uri = $album['artist_uri'];
	}
...
function queue_uris($uris, $randomly)
{
	...
	elseif(get_uri_type($uris) == 'album')
	{
	...
	if(empty($return))
	{
		...
		if(!empty($name[1]) && !empty($artists[1]) && !empty($titles[1]) && !empty($uris[1]) && !empty($lengths[1]))
		{
			...
			for($i = 0; $i < $count; $i++)
			{
				...
				$return['tracks'][$i]['length'] = $lengths[1][$i];

				$total_length = $total_length + floatval($lengths[1][$i]);
			}

			$return['tracks_count'] = count($uris[1]);
...
function lookup_album($uri)
{
	...
	else
	{
		...
		if(!empty($album['album']['name']) && !empty($album['album']['tracks']))
		{
			...
			$return['released'] = $album['album']['released'];
...
function get_external_files($uris)
{
	...
	for($i = 0; $i < $count; $i++)
	{
		...
		curl_setopt($ch[$i], CURLOPT_USERAGENT, 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:21.0) Gecko/20100101 Firefox/21.0');
